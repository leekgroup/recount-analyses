...
rawCors <- sapply(seq_len(nrow(gtexCounts)), function(ii) pcor(gtexCounts[ii, ],
    recountCounts[ii,]))
summary(rawCors)
...
gtexCounts2 <- log2(sweep(gtexCounts, MARGIN = 2, FUN = "/", 
    colSums(gtexCounts) / (4 * 10^7 )) + 1)
recountCounts2 <- log2(assay(rse_gene2, "counts")[rownames(recountCounts),
    colnames(recountCounts)] + 1)
normCors <- sapply(seq_len(nrow(gtexCounts2)), function(ii) 
    pcor(gtexCounts2[ii, ], recountCounts2[ii, ]))
summary(normCors)
```
