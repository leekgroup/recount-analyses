...
num_genes <- lapply(rowData(rse_jx_filt)$gene_id, function(x) length(x))
...
## There are a lot of NAs: junctions that do not map to geneIDs
jx_gene_id[1:1000]
...
rowData(rse_jx_filt)[double_jx, c('junction_id', 'gene_ids')]
...
# Comparision of gene, exon, and junction results
To compare findings at the gene, exon, and junction level, we obtained a single exon-level [or junction-level] p-value for each gene included at the gene-level analysis. To do this, we utilized Simes' rule [PMID: 23330866], such that for each gene included in the gene-level analysis, the p-values for exons [or junctions] within that gene were extracted and sorted. Each exon-level [or junction-level] p-value is then multiplied by the number of exons [or junctions] present within the gene. For each exon [or junction] (1,2...n), this quantity is divided by that exon's rank [ or junction's rank] (where 1=most significant exon [or junction] and n=least significant). The minimum value from this calcultion is assigned as the exon-level [or junction level] p-value at each gene.
...
## ERROR when calcualting q-values 
# q_jx_gene <- qvalue(p_jx_gene)$q

## Alternative for multiple comparison adjustment
q_jx_gene <- p.adjust(p_jx_gene)
...
## Concordance plots with different x-axis
...
par(mfrow = c(1, 1), font.lab = 1.5, cex.lab = 1.2, font.axis = 1.5,
    cex.axis = 1.2)
...
par(mfrow = c(1, 1), font.lab = 1.5, cex.lab = 1.2, font.axis = 1.5,
    cex.axis = 1.2)
plot(seq(1:length(p.mod1.sort[1:1000])), conc[1:1000], 
     type = 'l',
     xlim = c(0, 1000),
     ylim = c(0, 1000),
     xlab = 'ordered genes',
...
lines(seq(1:length(p.mod1.sort[1:1000])), conc[1:1000], col = 'black', lwd = 2)
```
