...
There are `r length(regions_counts_sig)` statistically significant regions detected (`q < 0.05`). 
...
## For each region, determine if region overlaps with a gene
...
There are `length(regions_counts_notingene[regions_counts_notingene$q.mod<0.05])` DERs that do not overlap with known genes.
