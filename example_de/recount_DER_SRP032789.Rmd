...
and `limma`. Results for each region are plotted using a volcano plot of the $log_2(fold change)$ and $-log_{10}(p-value)$.
...
```
There are `length(regions_counts_notingene[regions_counts_notingene$q.mod<0.05])` DERs that do not overlap with genes included in the gene-level analysis.

These range in size from `min(width(regions_counts_notingene))` bps to `max(width(regions_counts_notingene))` bps in length, with a median length of `median(width(regions_counts_notingene))` bps.
...
```{r visualizing-DERs-that-do-not-overlap-annotated-exons, fig.height = 8, fig.width = 8}
...
files <- SRP032789_urls$path[grepl('bw$', SRP032789_urls$url)]
...
which <- regions
fullCov <- fullCoverage(files = files, chrs =c('chr1', 'chr2', 'chr3', 'chr4', 'chr5', 'chr6', 'chr7', 'chr8', 'chr9', 'chr10', 'chr11', 'chr12', 'chr13', 'chr14', 'chr15', 'chr16', 'chr17', 'chr18', 'chr19', 'chr20', 'chr21', 'chr22', 'chrX', 'chrY'), which = which)
...
geneRegionCov <- getRegionCoverage(fullCov=fullCov, regions=regions,
	targetSize = 4e+07, totalMapped=colData(rse)$reads_aligned)
...
### Leo! Not sure we need to include this?
pdf('plots/DERs_notingene.pdf', h = 5, w = 7)
...
```
There are `length(regions_counts_notinANYgene[regions_counts_notinANYgene$q.mod<0.05])` DERs that do not overlap with any known genes.

These range in size from `min(width(regions_counts_notinANYgene))` bps to `max(width(regions_counts_notinANYgene))` bps in length, with a median length of `median(width(regions_counts_notinANYgene))` bps.
